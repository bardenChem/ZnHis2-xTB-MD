...
CO2_WATER_REPLACEMENT_NOTE = (
    "This condition was generated by explicitly removing one selected "
    "solvent water molecule and placing one CO2 molecule in the resulting "
    "local solvent cavity. This is a deliberately prepared initial "
    "condition and does not demonstrate spontaneous water displacement, "
    "CO2 binding, desolvation equilibrium, or a substitution mechanism. "
...
def validate_co2_source_pdb(
    ...
    *,
    ...
    if (
        len(atoms) < n_solute_atoms
        or (len(atoms) == n_solute_atoms and not allow_no_water)
    ):
        raise RuntimeError(
...
def co2_water_replacement_metadata(
    source_pdb: Path,
    source_metadata: dict,
    oxygen_atom_index: int,
) -> dict:
    """Identify one explicit water by its 1-based oxygen atom ordinal."""
    atoms = _validated_pdb_atoms(source_pdb, "CO2 water-replacement source")
    n_source_atoms = source_metadata["n_source_atoms"]
    n_solute_atoms = source_metadata["n_solute_atoms"]
    if len(atoms) != n_source_atoms:
        raise RuntimeError(
            f"Water-replacement source {source_pdb} has {len(atoms)} atoms; "
            f"expected {n_source_atoms}."
        )
    if oxygen_atom_index < 1:
        raise RuntimeError("Replacement water oxygen atom index must be >= 1.")
    if oxygen_atom_index > n_source_atoms:
        raise RuntimeError(
            f"Replacement water oxygen atom index {oxygen_atom_index} is "
            f"outside the source atom range 1-{n_source_atoms}."
        )
    if oxygen_atom_index <= n_solute_atoms:
        raise RuntimeError(
            f"Replacement water oxygen atom {oxygen_atom_index} belongs to "
            f"the first {n_solute_atoms} solute atoms."
        )
    selected_atom = atoms[oxygen_atom_index - 1]
    if selected_atom["element"] != "O":
        raise RuntimeError(
            f"Replacement water atom {oxygen_atom_index} is "
            f"{selected_atom['element']}, not O."
        )
    if oxygen_atom_index not in source_metadata["water_oxygen_indices"]:
        raise RuntimeError(
            f"Replacement atom {oxygen_atom_index} is not a validated water "
            "oxygen in the source metadata."
        )

    solvent_offset = oxygen_atom_index - 1 - n_solute_atoms
    triplet_start = n_solute_atoms + (solvent_offset // 3) * 3
    triplet = atoms[triplet_start:triplet_start + 3]
    water_atom_indices = list(range(triplet_start + 1, triplet_start + 4))
    if (
        len(triplet) != 3
        or sorted(atom["element"] for atom in triplet) != ["H", "H", "O"]
        or oxygen_atom_index not in water_atom_indices
    ):
        raise RuntimeError(
            f"Replacement atom {oxygen_atom_index} does not belong to one "
            "valid consecutive H2O triplet in the solvent region."
        )
    if any(index <= n_solute_atoms for index in water_atom_indices):
        raise RuntimeError("Replacement water overlaps the solute atom block.")

    lines = source_pdb.read_text().splitlines(keepends=True)
    oxygen_line = lines[selected_atom["line_index"]].rstrip("\r\n")
    resname = oxygen_line[17:20].strip() or None
    chain = oxygen_line[21:22].strip() or None
    resid = oxygen_line[22:26].strip() or None
    zinc_xyz = atoms[source_metadata["zinc_index"] - 1]["xyz"]
    oxygen_xyz = selected_atom["xyz"]
    zinc_to_oxygen = tuple(
        oxygen - zinc for oxygen, zinc in zip(oxygen_xyz, zinc_xyz)
    )
    return {
        "replacement_water_oxygen_atom_index": oxygen_atom_index,
        "replacement_water_atom_indices": water_atom_indices,
        "replacement_water_resname": resname,
        "replacement_water_resid": resid,
        "replacement_water_chain": chain,
        "replacement_water_oxygen_xyz_A": list(oxygen_xyz),
        "replacement_water_Zn_O_distance_A": math.dist(zinc_xyz, oxygen_xyz),
        "zinc_to_replacement_center_vector_A": list(zinc_to_oxygen),
    }


def _pdb_without_atom_ordinals_text(
    source_pdb: Path,
    removed_atom_indices: list[int],
) -> str:
    """Return PDB text with selected atom ordinals omitted, preserving order."""
    removed = set(removed_atom_indices)
    lines = source_pdb.read_text().splitlines(keepends=True)
    atom_ordinal = 0
    output = []
    for line in lines:
        if line.startswith(("ATOM  ", "HETATM")):
            atom_ordinal += 1
            if atom_ordinal in removed:
                continue
        output.append(line)
    if atom_ordinal < max(removed, default=0):
        raise RuntimeError("Cannot remove water atoms outside the source PDB.")
    return "".join(output)


def write_co2_source_without_replacement_water(
    source_pdb: Path,
    output_pdb: Path,
    replacement_metadata: dict,
) -> dict:
    """Write and validate the effective source after removing exactly one H2O."""
    removed = replacement_metadata["replacement_water_atom_indices"]
    if len(removed) != 3 or len(set(removed)) != 3:
        raise RuntimeError("Water replacement must remove exactly three atoms.")
    output_pdb.write_text(_pdb_without_atom_ordinals_text(source_pdb, removed))
    original_atoms = _validated_pdb_atoms(source_pdb, "original CO2 source")
    effective_atoms = _validated_pdb_atoms(
        output_pdb, "source after replacement-water removal"
    )
    expected_elements = [
        atom["element"] for index, atom in enumerate(original_atoms, start=1)
        if index not in set(removed)
    ]
    if len(effective_atoms) != len(original_atoms) - 3:
        raise RuntimeError("Water replacement did not remove exactly three atoms.")
    if _element_sequence(effective_atoms) != expected_elements:
        raise RuntimeError(
            "Removing the replacement water changed the order of other atoms."
        )
    return {
        "original_source_atom_count": len(original_atoms),
        "removed_atom_count": 3,
        "source_after_water_removal_atom_count": len(effective_atoms),
    }


def validate_co2_source_without_replacement_water(
    source_pdb: Path,
    effective_pdb: Path,
    replacement_metadata: dict,
):
    """Ensure an archived reduced source is exactly the requested removal."""
    expected = _pdb_without_atom_ordinals_text(
        source_pdb,
        replacement_metadata["replacement_water_atom_indices"],
    )
    if effective_pdb.read_text() != expected:
        raise RuntimeError(
            "Archived source_water_removed.pdb is not the exact requested "
            "water removal from source_medoid.pdb."
        )


def co2_water_replacement_in_centered_frame(
    centered_effective_pdb: Path,
    effective_source_metadata: dict,
    template_metadata: dict,
    replacement_metadata: dict,
    configuration: dict,
) -> dict:
    """Transform the removed-O cavity into the effective centered frame."""
    atoms = _validated_pdb_atoms(
        centered_effective_pdb, "centered water-replacement source"
    )
    if _element_sequence(atoms) != effective_source_metadata["element_sequence"]:
        raise RuntimeError(
            "Centered water-replacement source changed effective atom order."
        )
    zinc_xyz = atoms[effective_source_metadata["zinc_index"] - 1]["xyz"]
    vector = replacement_metadata["zinc_to_replacement_center_vector_A"]
    replacement_center = [
        zinc + offset for zinc, offset in zip(zinc_xyz, vector)
    ]
    zinc_oxygen_distance = replacement_metadata[
        "replacement_water_Zn_O_distance_A"
    ]
    shell_tolerance = PDB_COORDINATE_TOLERANCE_A
    if (
        zinc_oxygen_distance < configuration["shell_inner_A"] - shell_tolerance
        or zinc_oxygen_distance > configuration["shell_outer_A"] + shell_tolerance
    ):
        raise RuntimeError(
            "Selected replacement water is outside the requested Zn shell: "
            f"Zn-O = {zinc_oxygen_distance:.6f} A, shell = "
            f"[{configuration['shell_inner_A']:.6f}, "
            f"{configuration['shell_outer_A']:.6f}] A. Adjust the shell "
            "bounds explicitly; no different water will be selected."
        )
    result = dict(replacement_metadata)
    result.update({
        "zinc_xyz_A": list(zinc_xyz),
        "replacement_center_xyz_A": replacement_center,
        "replacement_radius_A": configuration["replacement_radius_A"],
        "replacement_CO2_index": 1,
        "replacement_carbon_global_atom_index": (
            co2_targeted_carbon_global_atom_index(
                effective_source_metadata, template_metadata
            )
        ),
    })
    return result
...
def co2_effective_source_metadata(
    packing_manifest: dict,
    original_source_metadata: dict,
) -> dict:
    """Return source metadata matching the atom block actually packed."""
    if (
        packing_manifest.get("configuration", {}).get("placement_mode")
        != "water-replacement"
    ):
        return original_source_metadata
    effective = packing_manifest.get("effective_source")
    if not isinstance(effective, dict):
        raise RuntimeError(
            "Water-replacement packing manifest has no effective-source metadata."
        )
    required = (
        "n_source_atoms", "n_solute_atoms", "n_waters", "zinc_index",
        "water_oxygen_indices", "element_sequence",
    )
    missing = [field for field in required if field not in effective]
    if missing:
        raise RuntimeError(
            "Water-replacement effective-source metadata is incomplete "
...
def co2_packmol_input(
    ...
    *,
    ...
    water_replacement: dict | None = None,
    source_structure_name: str | None = None,
) -> str:
    if source_structure_name is None:
        source_structure_name = (
            "source_water_removed_centered.pdb"
            if placement_mode == "water-replacement"
            else "source_centered.pdb"
        )
...
structure {source_structure_name}
  number 1
  fixed 0. 0. 0. 0. 0. 0.
end structure
"""
...
"""
    if placement_mode not in ("site-directed", "water-replacement"):
    ...
    if placement_mode == "site-directed":
        ...
        if water_replacement is None:
            raise ValueError(
                "water-replacement Packmol input requires replacement metadata."
            )
        target_x, target_y, target_z = water_replacement[
            "replacement_center_xyz_A"
        ]
        target_radius_A = water_replacement["replacement_radius_A"]
    targeted_block = f"""
structure co2.pdb
  number 1
  atoms {carbon_local_index}
...
def validate_co2_water_replacement_geometry(
    pdb_path: Path,
    effective_source_metadata: dict,
    template_metadata: dict,
    water_replacement: dict,
    coordinate_tolerance_A: float,
) -> dict:
    """Validate replacement CO2 against the translated removed-water cavity."""
    atoms = _validated_pdb_atoms(pdb_path, "water-replacement CO2 geometry")
    zinc_index = effective_source_metadata["zinc_index"]
    zinc_xyz = atoms[zinc_index - 1]["xyz"]
    replacement_center = tuple(
        zinc + offset for zinc, offset in zip(
            zinc_xyz,
            water_replacement["zinc_to_replacement_center_vector_A"],
        )
    )
    carbon_atom_index = co2_targeted_carbon_global_atom_index(
        effective_source_metadata, template_metadata
    ...
    carbon_xyz = atoms[carbon_atom_index - 1]["xyz"]
    zinc_carbon_distance = math.dist(zinc_xyz, carbon_xyz)
    center_carbon_distance = math.dist(replacement_center, carbon_xyz)
    replacement_radius_A = water_replacement["replacement_radius_A"]
    if center_carbon_distance > replacement_radius_A + coordinate_tolerance_A:
        raise RuntimeError(
            "Replacement CO2 carbon is outside the removed-water target "
            f"sphere: C-center = {center_carbon_distance:.6f} A, radius = "
            f"{replacement_radius_A:.6f} A, tolerance = "
            f"{coordinate_tolerance_A:.6f} A."
        )
    return {
        "replacement_CO2_index": 1,
        "replacement_carbon_atom_index": carbon_atom_index,
        "replacement_Zn_C_distance_A": zinc_carbon_distance,
        "replacement_center_C_distance_A": center_carbon_distance,
        "replacement_radius_A": replacement_radius_A,
        "replacement_center_in_current_frame_A": list(replacement_center),
...
def validate_co2_packed_geometry(
    ...
    if water_replacement is not None:
        validation.update(validate_co2_water_replacement_geometry(
            packed_pdb,
            source_metadata,
            template_metadata,
            water_replacement,
...
def co2_pack_configuration(
    ...
    if placement_mode == "site-directed":
    ...
    elif placement_mode == "water-replacement":
        oxygen_atom_index = getattr(
            args, "co2_replace_water_oxygen_atom", None
        )
        if oxygen_atom_index is None:
            raise RuntimeError(
                "water-replacement CO2 placement requires "
                "--co2-replace-water-oxygen-atom INT."
            )
        replacement_radius_A = getattr(args, "co2_replacement_radius", 1.0)
        if (
            not math.isfinite(replacement_radius_A)
            or replacement_radius_A <= 0.0
        ):
            raise RuntimeError("CO2 replacement radius must be finite and > 0 A.")
        replacement_metadata = co2_water_replacement_metadata(
            Path(source_metadata["source_pdb"]),
            source_metadata,
            oxygen_atom_index,
        )
        zinc_oxygen_distance = replacement_metadata[
        ...
        if (
            zinc_oxygen_distance
            < args.co2_shell_inner - PDB_COORDINATE_TOLERANCE_A
            or zinc_oxygen_distance
            > args.co2_shell_outer + PDB_COORDINATE_TOLERANCE_A
        ):
            raise RuntimeError(
                "Selected replacement water is outside the requested Zn "
                f"shell: Zn-O = {zinc_oxygen_distance:.6f} A, shell = "
                f"[{args.co2_shell_inner:.6f}, "
                f"{args.co2_shell_outer:.6f}] A. Adjust the shell bounds "
                "explicitly; no different water will be selected."
            )
        configuration.update({
            "replacement_water_oxygen_atom_index": oxygen_atom_index,
            "replacement_radius_A": replacement_radius_A,
...
def co2_water_replacement_from_configuration(
    source_pdb: Path,
    centered_effective_pdb: Path,
    source_metadata: dict,
    effective_source_metadata: dict,
    ...
    if configuration.get("placement_mode") != "water-replacement":
        return None
    selected_water = co2_water_replacement_metadata(
        source_pdb,
        source_metadata,
        configuration["replacement_water_oxygen_atom_index"],
    )
    return co2_water_replacement_in_centered_frame(
        centered_effective_pdb,
...
def co2_water_replacement_manifest_metadata(
    water_replacement: dict,
) -> dict:
    """Return the stable, user-facing replacement provenance fields."""
    return {
        "oxygen_atom_index_original": water_replacement[
            "replacement_water_oxygen_atom_index"
        ],
        "water_atom_indices_original": water_replacement[
            "replacement_water_atom_indices"
        ],
        "resname": water_replacement["replacement_water_resname"],
        "resid": water_replacement["replacement_water_resid"],
        "chain": water_replacement["replacement_water_chain"],
        "original_oxygen_xyz_A": water_replacement[
            "replacement_water_oxygen_xyz_A"
        ],
        "Zn_O_distance_A": water_replacement[
            "replacement_water_Zn_O_distance_A"
        ],
        "zinc_to_replacement_center_vector_A": water_replacement[
            "zinc_to_replacement_center_vector_A"
        ],
        "replacement_center_in_packmol_frame_A": water_replacement[
            "replacement_center_xyz_A"
        ],
        "replacement_radius_A": water_replacement["replacement_radius_A"],
        "replacement_CO2_index": water_replacement[
            "replacement_CO2_index"
        ],
        "replacement_carbon_global_atom_index": water_replacement[
            "replacement_carbon_global_atom_index"
        ],
    }
...
def validate_completed_co2_pack(
    ...
    effective_source_centered = stage_dir / "source_centered.pdb"
    source_structure_name = effective_source_centered.name
    water_replacement = None
    if placement_mode == "water-replacement":
        selected_water = co2_water_replacement_metadata(
            stage_dir / "source_medoid.pdb",
            source_metadata,
            expected_configuration["replacement_water_oxygen_atom_index"],
        )
        effective_source = stage_dir / "source_water_removed.pdb"
        effective_source_centered = (
            stage_dir / "source_water_removed_centered.pdb"
        )
        source_structure_name = effective_source_centered.name
        validate_co2_source_without_replacement_water(
            stage_dir / "source_medoid.pdb",
            effective_source,
            selected_water,
        )
        effective_source_metadata = validate_co2_source_pdb(
            effective_source,
            source_metadata["n_solute_atoms"],
            allow_no_water=True,
        )
        recorded_effective_source = manifest.get("effective_source")
        comparable_effective_source = dict(effective_source_metadata)
        comparable_effective_source.pop("source_pdb", None)
        if isinstance(recorded_effective_source, dict):
            recorded_effective_source = dict(recorded_effective_source)
            recorded_effective_source.pop("source_pdb", None)
        if recorded_effective_source != comparable_effective_source:
            raise RuntimeError(
                f"Cannot reuse {CO2_PACK_STAGE}: effective-source metadata "
                "differs from source_water_removed.pdb."
            )
        water_replacement = co2_water_replacement_from_configuration(
            stage_dir / "source_medoid.pdb",
            effective_source_centered,
            source_metadata,
            effective_source_metadata,
        ...
        expected_replacement_manifest = (
            co2_water_replacement_manifest_metadata(water_replacement)
        )
        if manifest.get("water_replacement") != expected_replacement_manifest:
            raise RuntimeError(
                f"Cannot reuse {CO2_PACK_STAGE}: recorded water_replacement "
                "metadata differs from the requested water and geometry."
            )
        if manifest.get("water_replacement_scientific_note") != (
            CO2_WATER_REPLACEMENT_NOTE
        ):
            raise RuntimeError(
                f"Cannot reuse {CO2_PACK_STAGE}: missing or incompatible "
                "water-replacement scientific note."
            )
        expected_composition = {
            "n_solute_atoms": source_metadata["n_solute_atoms"],
            "original_n_waters": source_metadata["n_waters"],
            "removed_n_waters": 1,
            "n_waters": source_metadata["n_waters"] - 1,
            "n_co2": expected_configuration["co2_count"],
            "original_source_atoms": source_metadata["n_source_atoms"],
            "effective_source_atoms": effective_source_metadata[
                "n_source_atoms"
            ],
            "n_total_atoms": (
                effective_source_metadata["n_source_atoms"]
                + 3 * expected_configuration["co2_count"]
            ),
        }
        if manifest.get("composition") != expected_composition:
            raise RuntimeError(
                f"Cannot reuse {CO2_PACK_STAGE}: recorded water-replacement "
                "composition is incompatible."
            )
    ...
    centered_source_atoms = _validated_pdb_atoms(
        effective_source_centered, "centered effective CO2 source"
    )
    zinc_xyz = centered_source_atoms[
        effective_source_metadata["zinc_index"] - 1
    ]["xyz"]
    expected_packmol_input = co2_packmol_input(
        ...
        water_replacement=water_replacement,
    ...
    if water_replacement is not None:
        ...
        for field in (
            "replacement_CO2_index",
            "replacement_carbon_atom_index",
            "replacement_Zn_C_distance_A",
            "replacement_center_C_distance_A",
            "replacement_radius_A",
            "replacement_valid",
        ):
            if recorded_validation.get(field) != validation.get(field):
                raise RuntimeError(
                    f"Cannot reuse {CO2_PACK_STAGE}: recorded "
                    f"water-replacement validation field {field} is "
                    "inconsistent."
                )
...
def run_co2_pack_stage(
    ...
    source_water_removed = stage_dir / "source_water_removed.pdb"
    source_water_removed_centered = (
        stage_dir / "source_water_removed_centered.pdb"
    )
    ...
    effective_source_pdb = source_local
    effective_source_centered = source_centered
    effective_source_metadata = source_metadata
    effective_source_centering = source_centering
    removal_counts = None
    water_replacement = None
    if configuration["placement_mode"] == "water-replacement":
        selected_water = co2_water_replacement_metadata(
            source_local,
            source_metadata,
            configuration["replacement_water_oxygen_atom_index"],
        )
        removal_counts = write_co2_source_without_replacement_water(
            source_local,
            source_water_removed,
            selected_water,
        )
        effective_source_pdb = source_water_removed
        effective_source_centered = source_water_removed_centered
        effective_source_metadata = validate_co2_source_pdb(
            effective_source_pdb,
            source_metadata["n_solute_atoms"],
            allow_no_water=True,
        )
        if effective_source_metadata["n_waters"] != source_metadata["n_waters"] - 1:
            raise RuntimeError(
                "Water replacement did not reduce the validated water count by one."
            )
        effective_source_centering = center_pdb(
            effective_source_pdb, effective_source_centered
        )
        water_replacement = co2_water_replacement_from_configuration(
            source_local,
            effective_source_centered,
    ...
    if water_replacement is not None:
        residue = " ".join(
            value for value in (
                water_replacement["replacement_water_resname"],
                water_replacement["replacement_water_resid"],
            )
            if value
        ) or "unavailable"
        removed_atoms = " ".join(
            str(index) for index in water_replacement[
                "replacement_water_atom_indices"
            ]
        )
        print(
            "       Replacement water O index: "
            f"{water_replacement['replacement_water_oxygen_atom_index']}"
        )
        print(f"       Replacement water residue: {residue}")
        print(
            "       Replacement water Zn-O    : "
            f"{water_replacement['replacement_water_Zn_O_distance_A']:.6f} A"
        )
        print(f"       Removed water atoms       : {removed_atoms}")
        print(f"       Original waters           : {source_metadata['n_waters']}")
        print(
            "       Final waters              : "
            f"{effective_source_metadata['n_waters']}"
        )
        print(
            "       Replacement radius        : "
            f"{water_replacement['replacement_radius_A']:.6f} A"
        )
        print("       Replacement CO2           : molecule 1")
        print(
            f"       Background CO2 molecules : {co2_count - 1}"
        )
    ...
    try:
        ...
        if water_replacement is not None:
            output_names.extend([
                source_water_removed.name,
                source_water_removed_centered.name,
            ])
            composition = {
                "n_solute_atoms": source_metadata["n_solute_atoms"],
                "original_n_waters": source_metadata["n_waters"],
                "removed_n_waters": 1,
                "n_waters": effective_source_metadata["n_waters"],
                "n_co2": co2_count,
                "original_source_atoms": source_metadata["n_source_atoms"],
                "effective_source_atoms": effective_source_metadata[
                    "n_source_atoms"
                ...
                "n_total_atoms": (
                    effective_source_metadata["n_source_atoms"] + 3 * co2_count
                ),
            }
            paths.update({
                "effective_source_pdb": source_water_removed.name,
                "effective_source_centered_pdb": (
                    source_water_removed_centered.name
                ),
            })
        manifest = {
            **runtime,
            ...
            "effective_source_centering": effective_source_centering,
        ...
        if water_replacement is not None:
            manifest["effective_source"] = effective_source_metadata
            manifest["water_removal_counts"] = removal_counts
            manifest["water_replacement"] = (
                co2_water_replacement_manifest_metadata(water_replacement)
            )
            manifest["water_replacement_scientific_note"] = (
                CO2_WATER_REPLACEMENT_NOTE
            )
        ...
        if water_replacement is not None:
            print(
                "       Replacement Zn-C         : "
                f"{packing_validation['replacement_Zn_C_distance_A']:.6f} A"
            )
            print(
                "       C-replacement-center     : "
                f"{packing_validation['replacement_center_C_distance_A']:.6f} A"
            )
            print("       Replacement valid        : true")
        ...
        if water_replacement is not None:
            failure["effective_source"] = effective_source_metadata
            failure["water_removal_counts"] = removal_counts
            failure["water_replacement"] = (
                co2_water_replacement_manifest_metadata(water_replacement)
            )
            failure["water_replacement_scientific_note"] = (
                CO2_WATER_REPLACEMENT_NOTE
            )
        ...
        raise RuntimeError(
            ...
            "remove a different/additional water or relax the packing "
...
def validate_co2_accommodated_geometry(
    ...
    expected_atoms = (
        ...
        if expected_atom_count is None else expected_atom_count
    )
...
def co2_accommodation_configuration(
    ...
    if packing_configuration.get("placement_mode") == "water-replacement":
        composition = packing_manifest["composition"]
        configuration.update({
            "placement_mode": "water-replacement",
            "effective_source_atom_count": composition[
                "effective_source_atoms"
            ],
            "n_total_atoms": composition["n_total_atoms"],
        })
    return configuration
...
def validate_completed_co2_accommodation(
    ...
    validate_co2_accommodated_geometry(
        ...
        expected_atom_count=expected_configuration.get("n_total_atoms"),
    )
    return manifest
...
def run_co2_accommodation_stage(
    ...
    *,
    ...
    effective_source_metadata = co2_effective_source_metadata(
        packing_manifest, source_metadata
    )
    ...
    try:
        ...
        materialize_relaxed_pdb(
            stage_dir,
            packing_manifest["composition"]["n_total_atoms"],
        ...
        geometry_validation = validate_co2_accommodated_geometry(
            ...
            template_metadata,
            co2_count,
            expected_atom_count=packing_manifest["composition"][
                "n_total_atoms"
            ],
        )
        finished_at = datetime.now(timezone.utc).isoformat()
        ...
        if effective_source_metadata is not source_metadata:
            manifest["effective_source"] = effective_source_metadata
        _write_json(stage_dir / "stage_manifest.json", manifest)
        mark_stage_done(stage_dir)
        if diagnostics["converged"] is False:
...
PACKING_SUMMARY_FIELDS = [
    ...
    "site_direction_valid", "replacement_water_oxygen_atom_index",
    "replacement_water_resid", "replacement_water_Zn_O_A",
    "replacement_radius_A", "replacement_CO2_index",
    "replacement_carbon_atom_index", "replacement_Zn_C_A",
    "replacement_center_C_A", "replacement_valid", "removed_n_waters",
    "final_n_waters", "shell_inner_A", "shell_outer_A", "n_atoms",
...
def rebuild_co2_summaries(system_dir: Path):
    ...
    for condition_dir in condition_dirs:
        ...
        if pack_manifest_path.is_file():
            ...
            water_replacement = manifest.get("water_replacement", {})
            placement_mode = config.get("placement_mode", "random-shell")
            site_directed = placement_mode == "site-directed"
            replaces_water = placement_mode == "water-replacement"
            packing_rows.append({
                ...
                "replacement_water_oxygen_atom_index": (
                    water_replacement.get("oxygen_atom_index_original")
                    if replaces_water else None
                ),
                "replacement_water_resid": (
                    water_replacement.get("resid")
                    if replaces_water else None
                ),
                "replacement_water_Zn_O_A": (
                    water_replacement.get("Zn_O_distance_A")
                    if replaces_water else None
                ),
                "replacement_radius_A": (
                    water_replacement.get("replacement_radius_A")
                    if replaces_water else None
                ),
                "replacement_CO2_index": (
                    validation.get("replacement_CO2_index")
                    if replaces_water else None
                ),
                "replacement_carbon_atom_index": (
                    validation.get("replacement_carbon_atom_index")
                    if replaces_water else None
                ),
                "replacement_Zn_C_A": (
                    validation.get("replacement_Zn_C_distance_A")
                    if replaces_water else None
                ),
                "replacement_center_C_A": (
                    validation.get("replacement_center_C_distance_A")
                    if replaces_water else None
                ),
                "replacement_valid": (
                    validation.get("replacement_valid")
                    if replaces_water else None
                ),
                "removed_n_waters": (
                    composition.get("removed_n_waters")
                    if replaces_water else None
                ),
                "final_n_waters": (
                    composition.get("n_waters")
                    if replaces_water else None
                ),
...
class StoreWithExplicitFlag(argparse.Action):
    """Store a CLI value and retain whether the option was explicitly used."""
...
def parse_args():
    ...
    co2.add_argument(
        "--co2-placement-mode",
        choices=["random-shell", "site-directed", "water-replacement"],
        default="random-shell",
        help=(
            "Initial CO2 Packmol placement: historical random shell, one "
            "site-directed molecule, or one selected-water replacement, "
            "with remaining CO2 in the random shell "
            "(default: random-shell)."
        ),
    )
    co2.add_argument(
    ...
    co2.add_argument(
        "--co2-replace-water-oxygen-atom",
        type=int,
        default=None,
        metavar="INT",
        help=(
            "1-based ordinal ATOM/HETATM index of the explicitly selected "
            "water O in --co2-source-pdb; required for water-replacement."
        ),
    )
    co2.add_argument(
        "--co2-replacement-radius",
        type=float,
        action=StoreWithExplicitFlag,
        default=1.0,
        metavar="ANGSTROM",
        help=(
            "Target-sphere radius around the removed water O position in A "
            "for water-replacement (default: 1.0)."
        ),
    )
    p.set_defaults(
        co2_target_radius_explicit=False,
        co2_replacement_radius_explicit=False,
    )
    co2.add_argument(
        "--co2-shell-inner",
        type=float,
...
def validate_args(args):
    ...
    if args.co2_shell_screen:
        ...
        if (
            args.co2_placement_mode == "water-replacement"
            and args.co2_replace_water_oxygen_atom is None
        ):
            raise SystemExit(
                "--co2-placement-mode water-replacement requires "
                "--co2-replace-water-oxygen-atom INT."
            )
        if args.co2_placement_mode == "water-replacement":
            ambiguous = []
            if args.co2_direction_atom is not None:
                ambiguous.append("--co2-direction-atom")
            if args.co2_target_distance is not None:
                ambiguous.append("--co2-target-distance")
            if (
                getattr(args, "co2_target_radius_explicit", False)
                or args.co2_target_radius != 1.5
            ):
                ambiguous.append("--co2-target-radius")
            if ambiguous:
                raise SystemExit(
                    "water-replacement does not use site-directed options: "
                    + ", ".join(ambiguous)
                    + "."
                )
        elif (
            args.co2_replace_water_oxygen_atom is not None
            or getattr(args, "co2_replacement_radius_explicit", False)
            or args.co2_replacement_radius != 1.0
        ):
            raise SystemExit(
                "--co2-replace-water-oxygen-atom and a non-default "
                "--co2-replacement-radius require "
        ...
        if (
            args.co2_replace_water_oxygen_atom is not None
            and args.co2_replace_water_oxygen_atom < 1
        ):
            raise SystemExit(
                "--co2-replace-water-oxygen-atom must be >= 1."
            )
        if (
            not math.isfinite(args.co2_replacement_radius)
            or args.co2_replacement_radius <= 0
        ):
            raise SystemExit("--co2-replacement-radius must be > 0.")
        if (
    ...
    if (
        ...
        or getattr(args, "co2_target_radius_explicit", False)
        or args.co2_target_radius != 1.5
        or args.co2_replace_water_oxygen_atom is not None
        or getattr(args, "co2_replacement_radius_explicit", False)
        or args.co2_replacement_radius != 1.0
